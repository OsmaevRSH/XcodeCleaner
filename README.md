...
лежат в другом месте, корни задаются через `defaults`:

```bash
defaults write dev.ltheresi.xcodecleaner ProjectSearchRoots -array "mobile/saft/ios" "mobile/music/ios"
```

Список заменяет значение по умолчанию целиком и читается при запуске.
Вернуть исходное поведение: `defaults delete dev.ltheresi.xcodecleaner
ProjectSearchRoots`.
...
swift test               # 167 тестов, около секунды
```
